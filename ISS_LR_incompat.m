% This is a script to create plots for PD-BT for the ISS1R model with an
% incompatible prior covariance.
...
[X,D]               = eig(Gamma_pr_full); % eigendecompostition of prior
[~,ind]             = sort(diag(D),'descend'); % get important directions
Ds                  = D(ind,ind);
Xs                  = X(:,ind);
L_pr_full           = Xs(:,1:prior_rank)*sqrt(Ds(1:prior_rank,1:prior_rank));
